...
public class AsciiDataToParticle_simple : MonoBehaviour {
    ...
    public int decalageAxiale = 5;




    // Read the data and store it in ATOM structure array
    public void ReadAsciiData() { //We associate with Valeur_sup and Y the index in the ATOM table corresponding to the color chosen as parameter in the table
        // Data path
            ...
            for (int _j = 0; _j < nb_bille; _j++) {
                if (_allLines[_i].Length > 0) {
                    ...
                    if (grey == false) {
                        ...
                        _Atoms[_k]._position = new Vector3(decalageAxiale * float.Parse(_data[0]), (CoefRouge * float.Parse(_data[3]) + CoefVert * float.Parse(_data[4]) + CoefBleu * float.Parse(_data[5])) * (_j * decalage_bille + offset), decalageAxiale * float.Parse(_data[1]));
                        ...
                        string[] _data = _allLines[_i].Split();
                        _Atoms[_k]._position = new Vector3(decalageAxiale * float.Parse(_data[0]), (0.7152f * float.Parse(_data[4]) + 0.0722f * float.Parse(_data[5]) + 0.2126f * float.Parse(_data[3])) * (_j * decalage_bille + offset), decalageAxiale*float.Parse(_data[1]));
                        _Atoms[_k]._radius = taille;
